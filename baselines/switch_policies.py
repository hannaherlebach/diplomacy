...
class PolicyUtils:
    ...
    def get_unit_adjacency_value(self, unit):
...
class SwitchPolicy:
    ...
    def actions(self, slots_list, observation, legal_actions):
        ...
        return actions, {'values': None, 'policy': None, 'actions': None}
    
        # Version that assumes switch condition is same for all powers; this works for switch_after_k_turns

        # if not self.switch_condition(self.turn_num, None, observation):
        #     print('Network policy')
...
def run_experiment(policies, slots_to_policies, max_length=5):
...
if __name__ == '__main__':
    ...
    sl_policy = get_network_policy_instance()
    switch_after_10_turns = partial(switch_after_k_turns, k=10)
    switch_after_20_turns = partial(switch_after_k_turns, k=20)
    random_disband_policy = RandomDisbandPolicy(num_players=7, p=0.5)
    mixed_policy_1 = SwitchPolicy(sl_policy, switch_after_10_turns, random_disband_policy)
    mixed_policy_2 = SwitchPolicy(sl_policy, switch_after_20_turns, random_disband_policy)
